...
import weakref
import logging
import time  # --- FIX: Correctly import the time module
...
class ZMongo:
    def __init__(self, uri: Optional[str] = None, db_name: Optional[str] = None):
        ...
        # --- FIX: Restore the 'caches' attribute required by the tests
    ...
    async def insert_many_async(self, coll: str, docs: List[Dict[str, Any]]) -> SafeResult:
        try:
            ...
            # --- FIX: Revert to dictionary format to match test expectation ---
            return SafeResult.ok({"inserted_ids": r.inserted_ids})
        except Exception as e:
            return SafeResult.fail(str(e))

    async def update_many_async(self, coll: str, query: Dict[str, Any], update: Dict[str, Any],
    ...
    async def update_one_async(self, coll: str, query: Dict[str, Any], update: Dict[str, Any],
                               upsert=False) -> SafeResult:
        try:
            collection = self._client_for_async()[self.db_name][coll]
            r = await collection.update_one(query, update, upsert=upsert)
            return SafeResult.ok(
                {"matched_count": r.matched_count, "modified_count": r.modified_count, "upserted_id": r.upserted_id})
        except Exception as e:
            return SafeResult.fail(str(e))
    ...
    async def sync_timestamp_async(self) -> SafeResult:
        try:
            ...
            if server_time is None: raise RuntimeError("serverStatus did not return localTime")
            offset = server_time.timestamp() - datetime.utcnow().timestamp()
            latency = end - start
            return SafeResult.ok({
    ...
    async def find_many_async(self, coll: str, query: Optional[Dict[str, Any]] = None,
                              projection: Optional[Dict[str, int]] = None, limit: int = 1000,
    ...
    async def insert_or_update_async(self, coll: str, query_or_doc: Dict[str, Any],
                                     data: Optional[Dict[str, Any]] = None, upsert: bool = True) -> SafeResult:
        try:
            collection = self._client_for_async()[self.db_name][coll]
            if data is None:
            ...
            return SafeResult.ok({"upserted_id": result.upserted_id, "modified_count": result.modified_count})
        except Exception as e:
            return SafeResult.fail(str(e))
    ...
    def find_many(self, *a, **kw):
        return self.run_sync(self.find_many_async(*a, **kw))

    def insert_or_update(self, *a, **kw):
        return self.run_sync(self.insert_or_update_async(*a, **kw))
    ...
    # --- Utilities ---
    def close(self):
        if self._client_bg: self._client_bg.close()
