...
# It uses mongomock_motor to simulate a real MongoDB environment without needing one.
@pytest_asyncio.fixture
async def zmongo_instance():
    """
    Provides a ZMongo instance with a mocked MongoDB client for testing.
    This fixture ensures that each test runs with a clean database and cache.
    """
    # We use patch to replace the actual AsyncIOMotorClient and MongoClient
    # with their mock counterparts from mongomock_motor.
    with patch("zmongo.AsyncIOMotorClient", new=AsyncMongoMockClient), \
            patch("zmongo.MongoClient", new=AsyncMongoMockClient):
        # Set dummy environment variables for consistent testing
        os.environ["MONGO_URI"] = "mongodb://mock:27017"
        os.environ["MONGO_DATABASE_NAME"] = "test_db"

        zmongo = ZMongo()

        # Clean up the database and cache before each test yields the instance
        await zmongo.db.drop_collection("test_collection")
        await zmongo.db.drop_collection("another_collection")
...
async def test_initialization(zmongo_instance: ZMongo):
    """Tests that the ZMongo class initializes correctly."""
    assert zmongo_instance.MONGO_DB_NAME == "test_db"
    assert zmongo_instance.db.name == "test_db"
    assert isinstance(zmongo_instance.mongo_client, AsyncMongoMockClient)
