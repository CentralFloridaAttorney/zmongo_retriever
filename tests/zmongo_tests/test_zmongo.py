...
# with a MONGO_URI and a MONGO_DATABASE_NAME for a dedicated test database.
# The tests will create and destroy data in this database.
#
# Example .env_local:
# MONGO_URI="mongodb://127.0.0.1:27017"
# MONGO_DATABASE_NAME="zmongo_test_db"
# ---
...
class TestZMongoIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        This runs before each 'test_*' method.
        """
        self.zm = ZMongo()
        self.db = self.zm.db  # Direct access to the motor database for verification
        # Create a unique collection name for each test to ensure isolation
        self.collection_name = self._testMethodName

        # FIX: Drop the collection at the start of each test to guarantee isolation.
        # This is the most robust way to prevent data from leaking between tests.
    ...
    async def asyncTearDown(self):
        ...
        This runs after each 'test_*' method.
        """
        # Teardown is still good practice as a final cleanup step.
    ...
    async def test_insert_and_find_document(self):
        ...
        # Find the document using ZMongo
    ...
    async def test_insert_many_and_count(self):
        ...
        # Verify the count using ZMongo's count method
    ...
    async def test_caching_behavior(self):
        ...
        # 2. Update the document directly in the DB, bypassing ZMongo's cache invalidation
        await self.db[self.collection_name].update_one({"_id": doc_id}, {"$set": {"version": 2}})

        # 3. Find again with cache=True. We should get the STALE, cached version (v1).
        ...
        # 4. Find again with cache=False. We should get the FRESH version from the DB (v2).
    ...
    async def test_cache_invalidation_on_update(self):
        ...
        # 2. Update the document using ZMongo. This should invalidate the cache.
        await self.zm.update_document(self.collection_name, {"_id": doc_id}, {"version": 2})

        # 3. Find the document again. We should get the new version (v2).
        find_res = await self.zm.find_document(self.collection_name, {"_id": doc_id})
