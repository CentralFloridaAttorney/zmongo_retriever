...
def zmongo_instance():
    ...
    # Teardown:
    # 1. Use the SAFE synchronous method to drop the database.
    # 2. THEN close the connection. This fixes the "Event loop is closed" error.
    zm.drop_database_sync(db_name)
    zm.close()
...
class TestZMongoCache:
    ...
    async def test_caching_and_invalidation(self, zmongo_instance):
        ...
        doc_id_str = "cache_doc_1"

        # 1. Insert and cache the document
        await zm.insert_one(collection, {"_id": doc_id_str, "version": 1})
        res1 = await zm.find_one(collection, {"_id": doc_id_str}, cache=True)
        assert res1.data["version"] == 1

        # 2. Directly update in DB to make cache stale
        await zm.db[collection].update_one({"_id": doc_id_str}, {"$set": {"version": 99}})

        # 3. Fetch again, should get stale data from cache
        res2_cached = await zm.find_one(collection, {"_id": doc_id_str}, cache=True)
        assert res2_cached.data["version"] == 1

        # 4. Update via ZMongo, which should invalidate the cache
        await zm.update_one(collection, {"_id": doc_id_str}, {"version": 2})

        # 5. Fetch again, should get the fresh data (version 2)
        res3_fresh = await zm.find_one(collection, {"_id": doc_id_str}, cache=True)
    ...
    async def test_cache_is_cleared_on_bulk_ops(self, zmongo_instance):
        ...
        # Cache a document
        await zm.insert_one(collection, {"_id": "doc1"})
