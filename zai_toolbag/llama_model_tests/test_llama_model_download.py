...
class TestLlamaModelDownload(unittest.TestCase):
    ...
    @patch.dict(os.environ, {'MODEL_PATH': '/tmp/test-model.bin'})  # Mock model path for the test
    ...
    @patch.dict(os.environ, {'MODEL_PATH': '/tmp/fake-path'})  # Mock model path for the test
    @patch("zmongo_toolbag.llama_model.Llama")
    def test_load_model_initializes_llm(self, mock_llama):
        model = LlamaModel()
        model.model_path = "/tmp/fake-path"

        # Mock the Llama constructor to prevent it from trying to load the actual model
